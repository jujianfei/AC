...
{
    /// <summary>
    /// 新建设备时选择设备类型的窗体（有设备类型分类列表）。
    /// </summary>
    public partial class SelectDeviceTypeForm : Form
    {
        ...
        private void LoadDeviceTypeSort()
        {
            this.tvSelectDeviceSort.Nodes.Clear();
            this.lvSelectDeviceType.Items.Clear();

            this.LoadDeviceTypeSort(this.tvSelectDeviceSort.Nodes, this.m_Application.DeviceTypeSort, "所有设备");
            this.tvSelectDeviceSort.ExpandAll();

            if (this.tvSelectDeviceSort.Nodes.Count > 0)
            {
                this.tvSelectDeviceSort.SelectedNode = this.tvSelectDeviceSort.Nodes[0];
            }
        }
        ...
        private void tvSelectDeviceSort_AfterSelect(object sender, TreeViewEventArgs e)
        {
