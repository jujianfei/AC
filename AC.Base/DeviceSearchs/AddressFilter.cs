...
namespace AC.Base.DeviceSearchs
{
    ...
    /// 按照设备通讯地址搜索设备。
    /// </summary>
    public class AddressFilter : IDeviceFilter
    {
        /// <summary>
        /// 按照设备通讯地址搜索设备。
        /// </summary>
        ...
        public AddressFilter(string addresss)
        {
        ...
        /// </summary>
        /// <param name="addresss"></param>
        public AddressFilter(ICollection<string> addresss)
        {
        ...
        {
            if (device.Address != null && this.Addresss != null && this.Addresss.Length > 0)
            {
                foreach (string strAddress in this.Addresss)
                {
                    if (device.Address.Equals(strAddress))
                    {
                        return true;
                    }
                }
        ...
        public string GetFilterSql(bool enableTableName)
        {
            if (this.Addresss != null && this.Addresss.Length > 0)
            {
                ...
                {
                    return (enableTableName ? Tables.Device.TableName + "." : "") + Tables.Device.DeviceAddress + "=" + Function.SqlStr(this.Addresss[0]);
                }
                else
                {
                    ...
                    foreach (string strAddress in this.Addresss)
                    {
                        if (lstAddress.Contains(strAddress) == false)
                        {
                            strSql += Function.SqlStr(strAddress) + ",";
                            lstAddress.Add(strAddress);
                        }
                    }
        ...
        public string GetFilterDescription()
        {
            if (this.Addresss != null && this.Addresss.Length > 0)
            {
                if (this.Addresss.Length == 1)
                {
                    return "设备通讯地址为" + this.Addresss[0];
                }
                else
                {
                    string strSql = "";
                    ...
                    strSql = "设备通讯地址是" + strSql.Substring(1) + "的设备";
                    return strSql;
                }
            }
            else
