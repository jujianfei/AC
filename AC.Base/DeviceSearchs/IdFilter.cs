...
namespace AC.Base.DeviceSearchs
{
    ...
    /// 按照设备编号搜索设备。
    /// </summary>
    public class IdFilter : IDeviceFilter
    {
        /// <summary>
        /// 按照设备编号搜索设备。
        /// </summary>
        ...
        /// 按照设备编号搜索设备。
        /// </summary>
        /// <param name="ids">一个或多个设备 ID 编号组成的字符串，如果是多个 ID 则使用“,”分隔。</param>
        ...
        {
            if (device.DeviceId > 0 && this.Ids != null && this.Ids.Length > 0)
            {
                foreach (int intId in this.Ids)
                {
                    if (device.DeviceId == intId)
                    {
                        return true;
                    }
                }
        ...
        public string GetFilterSql(bool enableTableName)
        {
            if (this.Ids != null && this.Ids.Length > 0)
            {
                ...
                {
                    return (enableTableName ? Tables.Device.TableName + "." : "") + Tables.Device.DeviceId + "=" + this.Ids[0];
                }
                else
                {
        ...
        public string GetFilterDescription()
        {
            if (this.Ids != null && this.Ids.Length > 0)
            {
                ...
                else
                {
                    ...
                    strSql = "设备编号是" + strSql.Substring(1) + "的设备";
                    return strSql;
                }
            }
            else
