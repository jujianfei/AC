...
{
    /// <summary>
    /// 使用 DL/T 645-2007 电力行业标准通信规约的设备驱动。
    /// </summary>
    public class DLT645_2007Drive : DLT645Drive
    {
        /// <summary>
        /// 读取电表指定数据项的数据。
        /// </summary>
        /// <param name="dataItem">数据项编码。例如：00000000、00010000、0001FF00、04000101、04000102等</param>
        ...
        /// 读取电表指定数据项的数据。
        /// </summary>
        /// <param name="dataItem">数据项编码，4字节。例如：04000102可以表示为 byte[]{0x04,0x00,0x01,0x02}</param>
        /// <returns>读取的该数据项的数据。（已减33H）</returns>
        public byte[] ReadData(byte[] dataItem)
        {
            byte[] bytData = this.SendData(DLT645_2007ControlCodeOptions.ReadData, Function.BytesReversal(dataItem), 0);
            if (bytData.Length >= 4)
            {
                byte[] bytTemp = new byte[bytData.Length - 4];

                for (int intIndex = 4; intIndex < bytData.Length; intIndex++)
                {
                    bytTemp[intIndex - 4] = bytData[intIndex];
                }
                return bytTemp;
            }
            else
