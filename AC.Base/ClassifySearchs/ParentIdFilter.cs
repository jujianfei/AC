...
{
    /// <summary>
    /// 按照指定的上级分类编号搜索隶属于该分类的所有下级分类。
    /// </summary>
    public class ParentIdFilter : IClassifyFilter
    {
        /// <summary>
        /// 按照指定的上级分类编号搜索隶属于该分类的所有下级分类。
        /// </summary>
        ...
        {
            if (classify.Parent != null && classify.Parent.ClassifyId > 0 && this.Ids != null && this.Ids.Length > 0)
            {
                foreach (int intId in this.Ids)
                {
                    if (classify.Parent.ClassifyId == intId)
                    {
                        return true;
                    }
                }
        ...
        public string GetFilterSql(bool enableTableName)
        {
            if (this.Ids != null && this.Ids.Length > 0)
            {
                ...
                {
                    return (enableTableName ? Tables.Classify.TableName + "." : "") + Tables.Classify.ParentId + "=" + this.Ids[0];
                }
                else
                {
                    string strSql = (enableTableName ? Tables.Classify.TableName + "." : "") + Tables.Classify.ParentId + " IN (";
                    foreach (int intId in this.Ids)
                    {
                        strSql += intId + ",";
                    }
        ...
        public string GetFilterDescription()
        {
            if (this.Ids != null && this.Ids.Length > 0)
            {
                ...
                else
                {
                    ...
                    strSql = "上级分类编号是" + strSql.Substring(1) + "的分类";
                    return strSql;
                }
            }
            else
