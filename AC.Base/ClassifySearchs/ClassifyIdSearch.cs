...
namespace AC.Base.ClassifySearchs
{
    ...
    /// 提供按照设定条件搜索指定分类ID的功能。
    /// </summary>
    public class ClassifyIdSearch : Searchs.SearchBase<IList<int>, int, IClassifyFilter, IClassifyOrder>
    {
        ...
        public ClassifyIdSearch(ApplicationClass application)
            : base(application)
        {
        ...
        {
            return new Searchs.SearchSelectColumn[] { new Searchs.SearchSelectColumn(Tables.Classify.TableName, new string[] { Tables.Classify.ClassifyId }) };
        }
            ...
            try
            {
                ...
                if (dr != null)
                {
                    while (dr.Read())
                    {
                        ...
                        if (lstId.Contains(intClassifyId) == false)
                        {
                            lstId.Add(intClassifyId);
                        }
                    }
                    dr.Close();
                }
