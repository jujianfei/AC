...
namespace AC.Base
{
    ...
    public abstract class DeviceMonthDataOrder<V> : Searchs.ISearch<IList<V>>
    {
        ...
        protected DeviceMonthDataOrder(ApplicationClass application, string dataTableName, params string[] tableNames)
        {
        ...
        /// <summary>
        /// 数据起始日期(必须)，使用该属性中的年、月部分。起始日期与结束日期必须为同年，且起始日期小于等于结束日期。
        /// </summary>
        public DateTime StartDate
        {
            ...
            set
            {
                ...
                {
                    throw new Exception("起始日期与结束日期必须是同年。");
                }
            }
        }
        ...
        /// <summary>
        /// 数据结束日期(必须)，使用该属性中的年、月部分。结束日期与起始日期必须为同年，且结束日期大于等于起始日期。
        /// </summary>
