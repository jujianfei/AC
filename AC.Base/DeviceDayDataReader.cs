...
namespace AC.Base
{
    ...
    /// 读取每月创建一张数据表的设备日数据。
    /// </summary>
    public abstract class DeviceDayDataReader
    {
        /// <summary>
        /// 读取每月创建一张数据表的设备数据。
        /// </summary>
        ...
        public DeviceDayDataReader(Device device, DateTime startDate, DateTime endDate, string tableName, string deviceIdColumnName, string dateNumColumnName)
        {
            ...
            if (dbConn != null)
            {
                try
                {
                    ...
                    {
                        string strTableName = Database.DbConnection.GetMonthlyName(tableName, range.Date.Year, range.Date.Month);
