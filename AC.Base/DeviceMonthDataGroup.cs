...
namespace AC.Base
{
    ...
    /// 对设备月数据进行分组聚合统计。
    /// </summary>
    public abstract class DeviceMonthDataGroup
    {
        ...
        /// <param name="dataTableName">储存月数据的数据表名。</param>
        /// <param name="tableNames">其它关联的表名</param>
        protected DeviceMonthDataGroup(ApplicationClass application, string dataTableName, params string[] tableNames)
        {
        ...
        /// <summary>
        /// 数据起始日期(必须)，使用该属性中的年、月部分。起始日期与结束日期必须同年，且起始日期小于等于结束日期。
        /// </summary>
        public DateTime StartDate
        {
            ...
            set
            {
                ...
                else if (this.m_StartDate.Year != this.m_EndDate.Year)
                {
                    throw new Exception("起始日期与结束日期必须同年。");
                }
            }
        }
        ...
        /// <summary>
        /// 数据结束日期(必须)，使用该属性中的年、月部分。结束日期与起始日期必须同年，且结束日期大于等于起始日期。
        /// </summary>
        ...
        {
            return Database.DbConnection.GetYearlyName(this.m_DataTableName, this.StartDate.Year);
        }
        ...
        /// </summary>
        /// <returns></returns>
        protected abstract string GetAggregateColumnName();
        ...
        protected System.Data.IDataReader GetDataReader()
        {
            ...
            if (this.DbConnection.TableIsExist(this.GetDataTableName()))
            {
                ...
                string strSql = "SELECT " + this.GetGroupColumnName() + "," + this.GetAggregateColumnName() + " FROM " + strFrom + strWhere + " GROUP BY " + this.GetGroupColumnName();
